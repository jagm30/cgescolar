...
class StorePlanPagoRequest extends FormRequest
{
    ...
    {
        $descuentos = collect($this->input('descuentos', []))
            ->filter(fn (array $descuento): bool => $this->hasDiscountPolicyData($descuento))
            ->values()
            ->all();

        $recargo = $this->input('recargo', []);

        $this->merge([
            'descuentos' => $descuentos ?: null,
            'recargo' => $this->hasSurchargePolicyData($recargo) ? $recargo : null,
        ]);
    }
    ...
    private function hasDiscountPolicyData(array $descuento): bool
    {
        return filled($descuento['nombre'] ?? null)
            || filled($descuento['valor'] ?? null)
            || filled($descuento['dia_limite'] ?? null);
    }

    private function hasSurchargePolicyData(mixed $recargo): bool
    {
        if (! is_array($recargo)) {
            return false;
        }

        return filled($recargo['dia_limite_pago'] ?? null)
            || filled($recargo['valor'] ?? null)
            || filled($recargo['tope_maximo'] ?? null);
    }
}
