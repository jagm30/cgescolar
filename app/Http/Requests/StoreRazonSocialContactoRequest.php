...
class StoreRazonSocialContactoRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            $total = RazonSocialContacto::where('contacto_id', $this->contacto_id)
                ->where('activo', true)
                ->count();
            ...
            $existe = RazonSocialContacto::where('contacto_id', $this->contacto_id)
            ...
            if (
                $this->filled('regimen_fiscal') && $this->filled('uso_cfdi_default')
                && ! RazonSocialContacto::usoCfdiCompatibleConRegimen($this->regimen_fiscal, $this->uso_cfdi_default)
            ) {
                $validator->errors()->add(
                    'uso_cfdi_default',
                    'El régimen fiscal '.$this->regimen_fiscal.' no admite deducciones personales (D01–D10). Elige otro Uso de CFDI, como G03.'
                );
            }
        });
    }
