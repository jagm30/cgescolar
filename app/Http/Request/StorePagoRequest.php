    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            foreach ($this->detalles ?? [] as $index => $detalle) {
                $cargo = \App\Models\Cargo::find($detalle['cargo_id'] ?? null);
                if (!$cargo) continue;
                ...
                // El abono no puede superar el saldo pendiente del cargo
                $saldoPendiente = $cargo->saldo_pendiente_base;
                if (isset($detalle['monto']) && $detalle['monto'] > $saldoPendiente + 0.01) {
                    $validator->errors()->add(
                        "detalles.{$index}.monto",
                        "El monto abonado (\${$detalle['monto']}) supera el saldo pendiente (\${$saldoPendiente}) del cargo #{$cargo->id}."
                    );
                }
            }
        });
